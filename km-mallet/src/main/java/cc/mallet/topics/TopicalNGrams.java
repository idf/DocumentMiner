...
public class TopicalNGrams {
	...
	public void printTopWords (int numWords, boolean useNewLines) {
		printTopWords(System.out, numWords, useNewLines);
	}

	public void printTopWords (PrintStream ps, int numWords, boolean useNewLines)
	{
		...
		for (int ti = 0; ti < numTopics; ti++) {
			...
			if (useNewLines) {
				...
				for (int i = 0; i < numToPrint; i++)
					ps.println (uniAlphabet.lookupObject(wp[i].wi).toString()
					                    + " " + wp[i].p/tokensPerTopic[ti]);
			} else {
				ps.print ("Topic "+ti+": ");
				for (int i = 0; i < numToPrint; i++)
					ps.print (uniAlphabet.lookupObject(wp[i].wi).toString() + " ");
			}
			...
			if (useNewLines) {
				ps.println ("\nTopic "+ti+" bigrams");
				for (int i = 0; i < numToPrint; i++)
					ps.println (biAlphabet.lookupObject(wp[i].wi).toString() + " " + wp[i].p/bisum);
			} else {
				ps.print ("          ");
				for (int i = 0; i < numToPrint; i++)
					ps.print (biAlphabet.lookupObject(wp[i].wi).toString() + " ");
				ps.println();
			}
			*/
			...
			//ps.println ("pre-sorting");
			int numNgrams = afv.numLocations();
			//ps.println ("post-sorting "+numNgrams);
			...
			if (useNewLines) {
				...
				for (int i = 0; i < Math.min(numNgrams,numWords); i++)
					ps.println (afv.getAlphabet().lookupObject(wp[i].wi).toString() + " " + wp[i].p/ngramSum);
			} else {
				ps.print (" (unigrams "+numUnitypeTokens+"/"+numUnitypeTypes+" bigrams "+numBitypeTokens+"/"+numBitypeTypes
				                  +" phrases "+Math.round(afv.oneNorm())+"/"+numNgrams+")\n         ");
				//ps.print (" (unique-ngrams="+numNgrams+" ngram-count="+Math.round(afv.oneNorm())+")\n         ");
				for (int i = 0; i < Math.min(numNgrams, numWords); i++)
					ps.print (afv.getAlphabet().lookupObject(wp[i].wi).toString() + " ");
				ps.println();
			}
  ...
  public void printDocumentTopics (PrintWriter pw, double threshold, int max)
  {
    ...
    for (int di = 0; di < topics.length; di++) {
      ...
      pw.print (ilist.get(di).getName().toString()); pw.print (' ');
