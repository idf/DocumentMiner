...
public class Vectors2Topics {
	...
	static CommandOption.String evaluatorFilename = new CommandOption.String(Vectors2Topics.class, "evaluator-filename", "FILENAME", true, null, "A held-out likelihood evaluator for new documents.  " + "By default this is null, indicating that no file will be written.", null);
	...
	static CommandOption.String topicReportXMLFile = new CommandOption.String(Vectors2Topics.class, "xml-topic-report", "FILENAME", true, null, "The filename in which to write the top words for each topic and any Dirichlet parameters in XML format.  " + "By default this is null, indicating that no file will be written.", null);

	static CommandOption.String topicPhraseReportXMLFile = new CommandOption.String(Vectors2Topics.class, "xml-topic-phrase-report", "FILENAME", true, null, "The filename in which to write the top words and phrases for each topic and any Dirichlet parameters in XML format.  " + "By default this is null, indicating that no file will be written.", null);
	...
	public static void main(String[] args) throws java.io.IOException {
		...
		param.add("--input");
		param.add(Setting.MalletSetting.OAC_69_FOLDER+"posts.mallet");
		param.add("--num-topics");
		param.add("10");
		param.add("--num-iterations");
		param.add("10");
		param.add("--output-topic-keys");
		param.add(Setting.MalletSetting.OAC_69_FOLDER+"10_keys.txt");
		param.add("--output-doc-topics");
		param.add(Setting.MalletSetting.OAC_69_FOLDER+"10_topics.txt");
		param.add("--topic-word-weights-file");
		param.add(Setting.MalletSetting.OAC_69_FOLDER+"10_weights.txt");
		param.add("--word-topic-counts-file");
		param.add(Setting.MalletSetting.OAC_69_FOLDER+"10_counts.txt");
		...
		if (usePAM.value) {
			...
			pam.estimate(ilist, numIterations.value, /* optimizeModelInterval */50, showTopicsInterval.value, outputModelInterval.value, outputModelFilename.value, randomSeed.value == 0 ? new Randoms() : new Randoms(randomSeed.value));
			...
			TopicalNGrams tng = new TopicalNGrams(numTopics.value, alpha.value, beta.value, gamma.value, delta.value, delta1.value, delta2.value);
			tng.estimate(ilist, numIterations.value, showTopicsInterval.value, outputModelInterval.value, outputModelFilename.value, randomSeed.value == 0 ? new Randoms() : new Randoms(randomSeed.value));

			PrintStream ps = new PrintStream(new File(topicKeysFile.value));
			tng.printTopWords(ps, topWords.value, true);
			ps.close();
			...
			if (inferencerFilename.value != null) {
				try {
					for (int language = 0; language < numLanguages; language++) {
						...
						ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(inferencerFilename.value + "." + language));
			...
			if (inputModelFilename.value != null) {
				...
				try {
					...
					System.err.println("Unable to restore instance list " + inputFile.value + ": " + e);
					System.exit(1);
				}
			...
			if (inferencerFilename.value != null) {
				try {
					...
					ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(inferencerFilename.value));
			...
			if (evaluatorFilename.value != null) {
				try {
					...
					ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(evaluatorFilename.value));
