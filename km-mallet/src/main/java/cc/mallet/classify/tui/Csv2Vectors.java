...
public class Csv2Vectors {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		param.add("--input");
		param.add(Setting.MalletSetting.POSTS_FOLDER+"posts_69.csv");
		param.add("--output");
		param.add(Setting.MalletSetting.OAC_69_FOLDER+"posts_bigram.mallet");
		param.add("--keep-sequence");
		param.add("--remove-stopwords");
		param.add("TRUE");
		// param.add("--keep-sequence-bigrams");
		// param.add("TRUE");
		// param.add("--help");
		...
		if (inputFile == null) {
			throw new IllegalArgumentException("You must include `--input FILE ...' in order to specify a" + "file containing the instances, one per line.");
		}
		...
		if (usePipeFromVectorsFile.wasInvoked()) {
			...
			if (keepSequenceBigrams.value) {
				...
				try {
					...
					throw new IllegalArgumentException("The token regular expression (" + tokenRegex.value + ") was invalid: " + pse.getMessage());
				}
			}
			...
			if (stoplistFile.wasInvoked()) {
				...
				TokenSequenceRemoveStopwords stopwordFilter = new TokenSequenceRemoveStopwords(stoplistFile.value, encoding.value, false, // don't include default list
				...
				TokenSequenceRemoveStopwords stopwordFilter = new TokenSequenceRemoveStopwords(false, keepSequenceBigrams.value);
		...
		instances.addThruPipe(new CsvIterator(fileReader, Pattern.compile(lineRegex.value), dataOption.value, labelOption.value, nameOption.value));
